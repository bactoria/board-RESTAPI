...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity handleAccountAlreadyExistException(AccountAlreadyExistException e) {
        log.error("handleAccountAlreadyExistException", e);
        final ErrorResponse response = ErrorResponse.fromErrorCode(ErrorCode.ACCOUNT_ALREADY_EXIST);
        return new ResponseEntity(response, HttpStatus.valueOf(response.getStatus()));
    }
}
